...
{
    public interface IReadableFromStream
    {
    ...
    public static class StreamExm
    {
        ...
        public static T[] ReadStructs<T>(this Stream input, int count) where T : new()
        {
            ...
            if ((TypeCache<T>.Flags & TypeCacheFlags.AllowUnsafeCasting) == TypeCacheFlags.AllowUnsafeCasting)
            {
                int size = TypeCache<T>.UnsafeSize;

                byte[] buff = new byte[size * count];
                input.EnsureRead(buff, 0, buff.Length);

                return buff.UnsafeInplaceCast<T>();
            }
