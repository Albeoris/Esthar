...
namespace Esthar.OpenGL
{
    public sealed class GLTexture : IDisposable
    {
        ...
        public void ToAct(string generalPath)
        {
            Exceptions.CheckArgumentNullOrEmprty(generalPath, "generalPath");

            ActPalleteFilesGLTextureWriter writer = new ActPalleteFilesGLTextureWriter(this, generalPath);
            writer.Write();
        }
        ...
        public void Draw(float x, float y, float z, float ox, float oy, float w, float h)
        {
            ...
            GL.TexCoord2(tx, ty);           GL.Vertex3(x, y, z);
            GL.TexCoord2(tx + tw, ty);      GL.Vertex3(x + w, y, z);
            GL.TexCoord2(tx + tw, ty + th); GL.Vertex3(x + w, y + h, z);
            GL.TexCoord2(tx, ty + th);      GL.Vertex3(x, y + h, z);
